...
def run(bot):
    ...
    async def dns(ctx):
        e = Embed(
            description=f'{ctx.author.mention}, we highly recommend using Wiimmfi, the most active service at the '
                        'moment.\n'
                        'You can see the current activity for Mario Kart DS with `/online` or by visiting the '
                        '[official website](https://wiimmfi.de/stats/game/mariokartds).\n\n'
